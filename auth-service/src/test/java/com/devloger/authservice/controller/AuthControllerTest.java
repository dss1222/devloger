...
class AuthControllerTest {
    ...
    class MeTest {
        ...
        void 내_정보_조회_성공() throws Exception {
            // given
            UserMeResponse response = new UserMeResponse(TEST_USER_ID, TEST_EMAIL, TEST_NICKNAME);
            when(userService.getById(TEST_USER_ID)).thenReturn(response);

            // when & then
            performMeRequest(TEST_USER_ID)
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.id").value(TEST_USER_ID))
                    .andExpect(jsonPath("$.email").value(TEST_EMAIL))
                    .andExpect(jsonPath("$.nickname").value(TEST_NICKNAME));
        ...
        @Test
        @DisplayName("내 정보 조회 실패 - X-User-Id 헤더 누락")
        void 내_정보_조회_실패_헤더_누락() throws Exception {
            // when & then
            mockMvc.perform(get(ME_URL))
                    .andExpect(status().isBadRequest());
        }

        private ResultActions performMeRequest(Long userId) throws Exception {
